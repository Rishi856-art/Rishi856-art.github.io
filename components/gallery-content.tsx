  ...
  return (
    <>
      ...
      <div className="page-shell min-h-screen pb-24 pt-36 sm:pt-44">
        <Reveal>
          <p className="eyebrow">{album ? "Album" : "The archive"}</p>
          <h1 className="display-title mt-5">
            {album ? album.title : <>All frames,<br /><em className="text-white/45">one place.</em></>}
          </h1>
          <p className="mt-8 max-w-lg text-sm leading-7 text-white/45">
            {album?.description || "A living collection of people, places, and passing moments. Select a frame to see it full screen."}
          </p>
        </Reveal>
        <div className="mt-14">
          <PhotoGrid photos={visiblePhotos} filters={!album} />
          {!visiblePhotos.length && (
            <p className="py-24 text-center text-white/40">
              {error ? "The gallery cannot load because the CMS backend is unavailable." : "This space is ready for Rishi's first upload."}
            </p>
          )}
        </div>
      </div>
