...
export function AdminDashboard() {
  ...
  return <div className="min-h-screen bg-[#090909] lg:grid lg:grid-cols-[250px_1fr]">
    <aside className="border-b border-white/10 bg-[#0c0c0c] p-5 lg:min-h-screen lg:border-b-0 lg:border-r lg:p-7"><div className="font-display text-2xl tracking-[.15em]">LUMEN<span className="text-gold">.</span></div><p className="mt-1 text-[9px] uppercase tracking-[.2em] text-white/30">Studio dashboard</p><nav className="mt-8 flex gap-2 overflow-x-auto lg:flex-col">{navigation.map(([value,label,Icon]) => <button key={value} onClick={() => setTab(value)} className={`flex min-w-max items-center gap-3 rounded-lg px-4 py-3 text-xs uppercase tracking-[.13em] transition ${tab === value ? "bg-gold text-ink" : "text-white/45 hover:bg-white/5 hover:text-white"}`}><Icon size={16}/>{label}</button>)}</nav><button onClick={signOut} className="mt-5 flex items-center gap-3 px-4 py-3 text-xs uppercase tracking-[.13em] text-white/35 hover:text-white lg:absolute lg:bottom-7"><LogOut size={16}/>Sign out</button></aside>
